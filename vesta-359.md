...
9. **Breakaway Area for Wiring Holes (for Bus Terminal)**
10. **Magnet**
...
If a timeout message is displayed on the webpage, it means the Wired Door Contact did not receive the signal from the Panel. Please check whether DC-23-BUS is connected properly to the Panel within appropriate wiring distance.
{% endhint %}
...
If there is no response from the Panel after the press of test button, it means the Panel did not receive the test signal from the device. Please check whether DC-23-BUS is connected properly to the Panel within appropriate wiring distance.
{% endhint %}
