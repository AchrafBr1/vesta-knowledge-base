...
<figure><img src=".gitbook/assets/Unknown image (393)" alt="" width="375"><figcaption></figcaption></figure>
...
&#x20;     Blue LED: (Please refer to LED Indicator description below for details)
...
1. On the Panel local webpage, go to the Edit Device page, input the PIR Camera configuration in the Sensor Setting section. Click OK to confirm.Please refer to the table below for configuration details. For example, if you want to disable Pet Immunity and set Sensitivity level to normal level, you can input 01.
...
2. Press the function button once on the PIR Camera to send a signal to the Control Panel, new settings will be applied immediately. If the button is not pressed, new settings will be applied upon next signal transmission, e.g. supervision signal or IR trigger signal.
...
<table><thead><tr><th width="69" data-type="checkbox"></th><th>It is recommended to install the PIR Camera in the following locations:</th></tr></thead><tbody><tr><td>true</td><td>Don’t aim the device at stairways on which the animals can climb.</td></tr><tr><td>true</td><td>At a position such that an intruder would normally move across the PIR’s field of view.</td></tr><tr><td>true</td><td>At a height between 2.3 and 2.5 meters above the ground for best performance.</td></tr><tr><td>true</td><td>In a corner to give the widest view.</td></tr><tr><td>true</td><td>At a position where its field of view will not be obstructed by, e.g., curtains, ornaments etc.</td></tr><tr><td>true</td><td>At a position where the animals cannot come to the detection area by climbing on furniture or other objects.</td></tr></tbody></table>
...
2. Break through the 2 Surface knockouts from the inside of the base.
...
<figure><img src=".gitbook/assets/Unknown image (413)" alt="" width="375"><figcaption></figcaption></figure>
