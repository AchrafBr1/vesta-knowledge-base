...
**Dome PIR Motion Sensor (IRD-23 / IRD-23SL)**

The Dome PIR Motion Sensor is designed to be mounted on the ceiling to provide a 360 ∘detection coverage without blind spots to pick up movements within an assigned area and signals the Control Panel to activate the alarm if an intruder crosses its’ path of detection.
...
The test button and also doubles as the LED indicator. The test button is used for testing the radio performance and for learning purpose. The LED indicator is used to indicate the status of system.
...
The Tamper switch protects the PIR from removal from mounted location.
...
*
  1. **Mounting Holes**
  2. **Hooks**
  3. **Ceiling Mounting Knockouts (Inside)**
* _**LED Indicator**_
...
*
  ...
  * When the cover is opened and the tamper switch is violated, the LED will light up for

2 sec. to indicate it is transmitting the “**Tamper**” signal.
...
The PIR is transmits a supervisory signal once every 30 to 50 minutes. If the Control Panel fails to receive the Supervisory signals transmitted from a certain PIR for a preset time, an “**Out-Of-Orde**r” fault message will be generated.
...
The batteries are included in the package. The PIR feature low battery detection function. A low battery signal will be sent to the Control Panel along with regular signal transmissions for the Control Panel to display the status accordingly.
...
* _**Getting Started**_
  ...
  * If the Control Panel receives PIR signal, it will display the information accordingly, refer to Control Panel manual to complete the learning process.
...
* _**Installation Guideline**_
  ...
  * When mounted at 4 meter height, the PIR provides detection coverage of a 360° circle wth approximately **8** meter diameter.
...
* **Limitations**
  * If a door is already protected by a Door Contact, do not install the PIR too close to the Door. If the Door Contac and the PIR is triggered and transmit signal at the same time, the signals may collide and cancel each other.
  * Do not install the PIR exposed to direct sunlight.
  * Avoid installing the PIR in areas where devices may cause rapid change of temperature in the detection area, i.e. air
...
* _**Mounting Method**_
  ...
  * After finishing PIR learning and walk test, proceed to mount the PIR according to instructions below I. Break through the 4 knockouts on inside of back cover

1. Use the knockout hoes as template, drill holes in the ceiling, insert wall plug if required.

III. Screw the back cover onto the ceiling according to drilled holes.

IV. The back cover has numbers on the insides to mark the mounting hole number, which corresponds to the mounting hook number on the back of main body. Align the mounting hole with the mounting hook when installing. Refer to figure below for mounting hook and hole location.
