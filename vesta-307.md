...
7. " \* " **Key -** Enter Keypad PIN Code (Default: **0000**) +  Key to enter Programming Mode.
...
* Press ＊ key and then **9** key — to enable Arm/ Home with Control Panel User Code function with a long beep checked by panel) (**Default**).
* Press “#” key **twice** — to exit Programming Mod&#x65;**.**
...
* **Enter Arm Mode**: Press "ARM AWAY"  key. If panel has no fault and arming is successful, the Red LED will turn ON with a short beep. When successful Arm, Red LED flashes, Backlight turns on with a long beep.
* **Enter Home Mode**: Press "ARM STAY" key. If panel has no fault and arming is successful, the Orange LED will turn ON with a short beep. When successful Home Arm, Orange LED flashes, Backlight turns on with a long beep.
...
2. Entry Delay —The keypad continuously beeps. When entering Disarm Mode, the Keypad will emit 2 beeps.
3. Alarm siren — Siren will sound continuously when alarm is triggered. Fire Alarm will sound 2 seconds and 1 second off.
...
_**Smart Home Sec APP**_ is used for Bluetooth pairing the smartphone with the keypad. Up to **25** smartphones can be paired with the keypad to disarm the system via _**Smart Home Sec APP**_ .

Please go to APP store or Google Play and search _**Smart Home Sec APP**_  to download the application.
...
**Step 2.** On your smartphone, open the _**Vesta Home 5 App**_, enter the registered user ID and password, and tap Connect to log into your account. _Note:_ Make sure Bluetooth is turned on for your smartphone setting.
...
To edit the Pairing PIN Code, enter 6-digit PIN Code and ＊ on the Keypad under the BLE Pairing mode. If the PIN Code setting is confirmed, the Keypad emits 1 long beep. If the PIN Code setting is not confirmed, the Keypad emits 4 beeps.
{% endhint %}
...
* The smartphone’s Bluetooth function needs to be turned ON, and the Smart Home Sec App must be kept active in the background so that the device can communicate with the App.
* A 4-digit or 6-digit Control Panel User Code must have been saved in the Smart Home Sec APP.
...
4. Close the case using the bottom fixing screw.
